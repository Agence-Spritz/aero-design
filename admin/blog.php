...
// PHOTOS
$photosize = "1170x674"; // Dimensions idéales d'information pour la photo

// le repertoir du dossier où m'on range les images, tout peut etre ds le même dossier
$chemin = "../images/pages-mobilier-aviation-objet-art-avion-objet-aviateur/";  // "/" à la fin
$wmax = 100; $hmax = 80;  $tdvisuphoto = $wmax*2+20;  	// Dimension pour affichage des vigettes
$redim_w=1170; $redim_h=674;
				...
				<form method="post" action="" enctype="multipart/form-data">
				    ...
				    <input type="hidden" name="page" value="<?=$lapage?>">
							...
							<div class="col-sm-12 col-md-6">
							        ...
							        <h4><i class='fa fa-pencil-square-o '></i> Rubrique</h4>
							        <div class="form-group">
								        <select name="<?=$chps[7]?>" class="form-control" />
							          
							             <?
							            $result = mysqli_query($link, "SELECT rub FROM $tableencours WHERE page='actu' GROUP BY rub ORDER BY rub") ;
							          
							            while ( list($list) = mysqli_fetch_array($result) ) 
							            {	echo " <option value=\"$list\" ";
							                if ( $list==$$chps[7] ) { print(" selected"); } 
							                echo " >".$list."</option>";
							            }		?>
								        </select>
								        
								        <?php if($masquer_rubrique!=1) {?>
								        <h4><i class='fa fa-calendar '></i> Ou nouvelle rubriques :</h4>
								        
								        <input name="rubnew" value="" class="form-control" type="text" />
								        <?php } ?>
								        
							        </div>
							        
							        <h4><i class='fa fa-pencil-square-o '></i> Tags (séparer par une virgule)</h4>
							        <div class="form-group">
							...
							<div class="col-sm-12 col-md-12 texte-principal">
						      ...
						      <h4><i class='fa fa-align-justify '></i> Texte secondaire</h4>
						      <textarea name="<?=$chps[8]?>" row contenu-admins="10" cols="50" ><?=$$chps[8]?></textarea><script type="text/javascript">CKEDITOR.replace( '<?=$chps[8]?>' );</script>
				...
				<div class="col-sm-12 col-md-12">
						...
						<tbody>
						  ...
						  { 
						    ...
						    echo "<td>$ID - $lg</td><td>".$rub." <i class='fa fa-angle-right fa-1x'></i></td><td>".strip_tags($titre)."</td><td>".substr(strip_tags($texte),0,60)."...</td><td>".date_barre($dbu)."</td><td><a href=\"?del=$ID&word=$word\"><i class='fa fa-trash-o '></i></a></td>";
						    echo "</tr>";
						  }
						 ?>
						</tbody>
