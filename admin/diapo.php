...
// PHOTOS
$photosize = "1920x1080";									// Dimensions idéales d'information pour la photo
$chemin = "../images/slides/";  							// "/" à la fin
$wmax = 100; $hmax = 80;  $tdvisuphoto = $wmax*2+20;  	// Dimension pour affichage des vigettes
$redim_w=1920; $redim_h=1080;

$masquervignette=0;
$nbr=0; // Nombre de photos

// CHAMPS
$chps=array('page','titre','texte','dbu','masquer','lg','rub','texte2','texte3');
$chpsNb = count($chps);
?>
...
{	if ( $modif ) 
	...
	unset($ID,$titre,$texte,$dbu,$masquer,$lg,$rub, $texte2, $texte3);
...
{	$result = mysqli_fetch_array( mysqli_query($link, " SELECT ID,$liste1 FROM $tableencours WHERE ID=$modif ") );
	list($ID,$page,$titre,$texte,$dbu,$masquer,$lg,$rub,$texte2, $texte3 ) = $result;
					...
					<form method="post" action="" enctype="multipart/form-data">
					    ...
					    <input type="hidden" name="rub" value="<?=$rub?>">
							...
							<div class="col-sm-12 col-md-6">
							        ...
							        <h4><i class='fa fa-pencil-square-o '></i> Nom diapo</h4>
							        <div class="form-group">
								        <input name="<?=$chps[1]?>" value="<?=$$chps[1]?>" class="form-control" type="text" required  />
							        </div>
							        
							        <h4><i class='fa fa-pencil-square-o '></i> Grand titre</h4>
							        <div class="form-group">
								    ...
								    <h4><i class='fa fa-eye '></i> Opacité foncée</h4>
								    <div class="form-group">
							          <select name="<?=$chps[8]?>" class="form-control">
							            <option value="0"<?php if ( $$chps[8]=="0" ) { print(" selected"); } ?>>Non</option>
							            <option value="1"<?php if ( $$chps[8]=="1" ) { print(" selected"); } ?>>Oui</option>
							          </select>
							        </div>
				...
				<div class="col-sm-12 col-md-12">
					...
					?>
					  	<table style="margin-top: 25px;" class="table table-bordered table-striped">
							...
							<tbody>
							  <?php 
							  while ( list($ID,$page,$titre,$texte,$dbu,$masquer,$lg,$rub, $texte2,$texte3) = mysqli_fetch_array($result) ) 
							  { 
